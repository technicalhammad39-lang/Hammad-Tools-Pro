...
const missingFirebasePublicEnv = requiredFirebasePublicEnv.filter((key) => {
  const value = process.env[key];
  return !value || !value.trim();
});

if (missingFirebasePublicEnv.length) {
  console.warn(
    `[next-config] Missing Firebase public env vars: ${missingFirebasePublicEnv.join(', ')}`
  );
}
